...
func _ready() -> void:
	...
	for i in range(1, 10):
		for j in range(1, 10):
			var specimen = GlobalSpeciesRegistry.generate_shroom(i * 10 + j)
			for model in specimen.get_children():
				model.set_position(model.get_position() + Vector3(-i * 10, 0.0, -j * 10))
			add_child(specimen)
